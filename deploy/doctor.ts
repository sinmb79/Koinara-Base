...
async function main(): Promise<void> {
  ...
  const deployManifestV2Path = resolve(ROOT, "deployments", `base-${profile}-v2.json`);
  if (!existsSync(deployManifestV2Path)) {
    warnings.push(`No v2 deployment manifest yet at deployments/base-${profile}-v2.json`);
  }
  ...
  console.log(`Koinara-Base doctor for ${profile}`);
